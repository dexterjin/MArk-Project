...
with open(f'./tweet_load.csv', 'r') as fr:
    ...
    buf2 = ['']
    with open(f'./tweet_load_predict.csv', 'w') as fw:
    ...
    for row in reader:
        ...
        with open(f'./tweet_load_predict.csv', 'a') as fw:
            if (reader.line_num-2) % 50 == 0:
                buf2 = ['']
#                fw.write("\"" + row['time'] +"\"" + "," + row['tweets'] + ":" + str(buf[reader.line_num-1]) + ":" + str(buf[reader.line_num]) + "\n")
                ...
                fw.write("\"" + row['time'] +"\"" + "," + str(buf2[reader.line_num % 50 -2]) + "\n")
