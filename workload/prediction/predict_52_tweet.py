...
with open(f'./tweet_load.csv', 'r') as fr:
    ...
    buf2 = ['']
    with open(f'./tweet_load_predict_10min.csv', 'w') as fw:
    ...
    for row in reader:
        ...
        with open(f'./tweet_load_predict_10min.csv', 'a') as fw:
            if (reader.line_num-2) % 10 == 0:
                buf2 = ['']
#                fw.write("\"" + row['time'] +"\"" + "," + row['tweets'] + ":" + str(buf[reader.line_num-1]) + ":" + str(buf[reader.line_num]) + "\n")
                ...
                fw.write("\"" + row['time'] +"\"" + "," + str(buf2[reader.line_num % 10 -2]) + "\n")
